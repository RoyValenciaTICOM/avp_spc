            ...
            <table class="table table-striped table-hover table-index">
              <thead>
                <tr>                 
                                  ...
                                  <th><?= $this->Paginator->sort('clave_asesor') ?> <i class="fa fa-sort fa-1"></i></th>
                                  <th><?= $this->Paginator->sort('co_group_id','Grupo') ?> <i class="fa fa-sort fa-1"></i></th>
                                  <th><?= $this->Paginator->sort('centros_captura_id','Centro Captura') ?> <i class="fa fa-sort fa-1"></i></th>
          ...
          <?php foreach ($coUsers as $coUser): ?>
            <tr>
                ...
                <td><?= $coUser->has('co_group') ? $this->Html->link($coUser->co_group->name, ['controller' => 'CoGroups', 'action' => 'view', $coUser->co_group->co_group_id]) : '' ?></td>
                 <td><?= $coUser->has('centros_captura') ? $this->Html->link($coUser->centros_captura->descripcion, ['controller' => 'CentrosCapturas', 'action' => 'view', $coUser->centros_captura->id]) : '' ?></td>
